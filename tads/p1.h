...
size_t node_counter(Abin<t>&a,typename Abin<t>::nodo n){
    ...
    return  1 + node_counter(a,a.hijoDrchoB(n)) + node_counter(a,a.hijoIzqdoB(n));
}
template <typename t>
size_t ej1(Abin<t>&a){
    return node_counter(a,a.raizB());
}
...
size_t ej3(typename Abin<t>::nodo n,Abin<t>&a){
    if (n==a.raizB())
        return 0;
    return 1+ ej3(a.padreB(n),a); //La profundidad sera el numero de padres que tiene el nodo (hasta llegar a la raiz)
}
...
size_t ej_6(Abin<t>& a){
    return desequilibrio(a.raizB(),a);//Llamada al desequilibrio de las alturas
}
...
bool pseudo_completo(typename  Abin<t>::nodo n, Abin<t>&a){
    ...
    pseudo_completo(a.hijoIzqdoB(n),a); //RECORRIDO INORDEN

    if (altura_rec(n,a)==1 and n!=a.raizB() ){ //HEMOS LLEGADO AL PENULTIMO NIVEL, ya que el penultimo nivel debe tener altura 1
        auto nodo_d=a.hijoDrchoB(a.padreB(n));
        auto nodo_i=a.hijoIzqdoB(a.padreB(n));
        //Comprobamos si es pseudo completo
        if ((a.hijoIzqdoB(nodo_d)!=Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_d)!=Abin<t>::NODO_NULO and
                a.hijoIzqdoB(nodo_i)==Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_i)==Abin<t>::NODO_NULO)
        or(a.hijoIzqdoB(nodo_i)!=Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_i)!=Abin<t>::NODO_NULO and
           a.hijoIzqdoB(nodo_d)==Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_d)==Abin<t>::NODO_NULO)
           or (a.hijoIzqdoB(nodo_d)==Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_d)==Abin<t>::NODO_NULO and
               a.hijoIzqdoB(nodo_i)==Abin<t>::NODO_NULO and a.hijoDrchoB(nodo_i)==Abin<t>::NODO_NULO)
           )
        {
            return true;
    ...
    pseudo_completo(a.hijoDrchoB(n),a);
}

template <class t>
bool ej_7(Abin<t>&a){
    return pseudo_completo(a.raizB(),a);
}
